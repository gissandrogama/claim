defmodule ClaimWeb.Admin.SessionControllerTest do
  ...
  describe "create/2" do
    ...
    test "returns 200 when admin credentials are valid", %{conn: conn} do
      conn = post(conn, "/api/v1/admin/sign_in", %{"email" => "kiba@gmail.com", "password" => "123456"})

      assert %{"status" => "ok", "data" => %{"name" => "Kiba Smith"}} = json_response(conn, 200)
    end

    test "returns 401 when admin email is invalid", %{conn: conn} do
      conn = post(conn, "/api/v1/admin/sign_in", %{"email" => "fom@gmail.com", "password" => "123456"})

      assert %{"status" => "unauthenticated"} = json_response(conn, 401)
    end

    test "returns 401 when admin password is invalid", %{conn: conn} do
      conn = post(conn, "/api/v1/admin/sign_in", %{"email" => "kiba@gmail.com", "password" => "1234567"})
